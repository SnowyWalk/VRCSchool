...
export function PageBreadcrumb({
    ...
    const parseCrumbLabel = (name: string) => {
        const m = name.match(/^\s*\{\s*img:(.+?)\s*\}\s*(.*)\s*$/);
        if (!m) return null;
        ...
        return {iconSrc, label};
    };

    const renderCrumbContent = (name: React.ReactNode) => {
        if (typeof name !== "string") {
            return null;
        }
        const parsed = parseCrumbLabel(name);
                ...
                return (
                    <Fragment key={i}>
                        ...
                        {item.url ? (
                            ...
                            >
                                {renderCrumbContent(item.name)}
                            </Link>
                        ) : (
                            <span className={className}>{renderCrumbContent(item.name)}</span>
                        )}
                    </Fragment>
                );
            })}
